...
async function load({ maxFaces = 10, inputWidth = 128, inputHeight = 128, iouThreshold = 0.3, scoreThreshold = 0.75, modelUrl = BLAZEFACE_MODEL_URL } = {}) {
    const blazeface = await tfconv.loadGraphModel(modelUrl);
    const model = new BlazeFaceModel(blazeface, inputWidth, inputHeight, maxFaces, iouThreshold, scoreThreshold);
    return model;
}
